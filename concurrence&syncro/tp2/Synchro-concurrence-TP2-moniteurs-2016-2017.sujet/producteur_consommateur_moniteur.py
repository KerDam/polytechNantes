...
class MoniteurProdCons () :
    def __init__(self, tampon) :
        ...
        self.conditionVide = threading.Condition(self.lock)
        self.conditionPlein = threading.Condition(self.lock)
...
def moniteur_deposer(moniteur, nom, element)  :
    ...
    if (tampon_est_plein(moniteur.tampon)):
        moniteur.conditionPlein.wait()
...
def moniteur_retirer(moniteur, nom)  :
    ...
    if (tampon_est_vide(moniteur.tampon)):
        moniteur.conditionVide.wait()
    ...
    moniteur.conditionPlein.notify()
